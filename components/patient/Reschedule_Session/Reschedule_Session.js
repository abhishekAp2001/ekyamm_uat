...
const Reschedule_Session = () => {
  ...
  const dateTime = searchParams.get("reschedule")
  ...
  const [selectedDate, setSelectedDate] = useState(convertToLocalDate(dateTime));
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_BASE_URL}/v2/cp/patient/session/update`,
        payload,
        headers
      );
          ...
          <div className="flex flex-col gap-6 font-quicksand text-sm bg-gradient-to-br rounded-[16px]">
            <div>
              ...
              <Input
                ...
                readOnly
                className="w-full h-10 rounded-[7.26px] bg-[#ffffff28] px-3 py-2  border border-[#E6E6E6] text-gray-400 text-[12px] font-semibold focus:border-none focus:ring-0 focus-visible:border-none focus-visible:ring-0 focus-visible:outline-0"
              />
            </div>
