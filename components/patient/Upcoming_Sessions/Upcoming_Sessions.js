...
const Upcoming_Sessions = ({ dashboard = false }) => {
  ...
  return (
    ...
    <div className={`relative max-w-[576px] overflow-auto h-full  flex flex-col ${pathname === '/patient/dashboard' ? '' : 'bg-[linear-gradient(#e2d9f6_0%,_#f9ccc587_100%)]'}`}>
      ...
      {/* Scrollable Body */}
      <div
        className={`flex-1 px-4 pb-5 ${pathname === "/patient/dashboard" ? "" : "pt-[15%] md:pt-[10%]"
          }`}
      >
        {loading ? (
          <div className="flex justify-center items-center h-40">
            <Loader2 className="w-6 h-6 animate-spin text-[#776EA5]" />
          </div>
        ) : (
          <>
            {/* Filter Row */}
            ...
            {sessions?.upcomingSessions && (
              <UpcomingSession
                ...
                pastSession={
                  sessions?.pastSessions?.[sessions?.pastSessions?.length - 1]
                }
              />
            )}

            {/* Past Sessions */}
            <PastSessions sessions={sessions?.pastSessions} />
          </>
        )}
      </div>
